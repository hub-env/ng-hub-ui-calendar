...
The hour ruler and the all-day strip that sits above it. The strip is drawn whether or not
the period holds an all-day event, so its label stays put and the hour grid never jumps by a
row; it shares the ruler's width so the label lines up with the hours below it.
